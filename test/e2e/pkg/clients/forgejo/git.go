...
func (fc *ForgejoClient) ForkRepository(sourceProjectID, targetProjectID string) (*forgejo.Repository, error) {
	...
	err := utils.WaitUntilWithInterval(func() (done bool, err error) {
		...
		if err != nil {
			...
			// Stale repo from a previous attempt or test run — delete and retry
			if statusCode == http.StatusConflict || strings.Contains(err.Error(), "already exists") {
				fmt.Printf("Target repo %s already exists, deleting stale repo and retrying\n", targetProjectID)
				if delErr := fc.DeleteRepositoryIfExists(targetProjectID); delErr != nil {
					fmt.Printf("Warning: failed to delete stale repo %s: %v\n", targetProjectID, delErr)
				}
			}
			return false, nil
		}
