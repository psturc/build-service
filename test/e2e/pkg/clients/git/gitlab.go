...
func NewGitlabClient(gl *gitlab.Client) *GitLabClient {
	return &GitLabClient{gl}
}

func (g *GitLabClient) CreateBranch(repository, baseBranchName, _, branchName string) error {
	return g.Client.CreateBranch(repository, branchName, baseBranchName)
}
...
func (g *GitLabClient) CreateFile(repository, pathToFile, content, branchName string) (*RepositoryFile, error) {
	_, err := g.Client.CreateFile(repository, pathToFile, content, branchName)
	if err != nil {
		return nil, err
	}
...
func (g *GitLabClient) ForkRepository(sourceRepoName, targetRepoName string) error {
	...
	_, err := g.Client.ForkRepository(sourceComponents[0], sourceComponents[1], targetComponents[0], targetComponents[1])
	if err != nil {
		return err
	}
	return nil
